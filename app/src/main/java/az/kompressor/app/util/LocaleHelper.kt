...
object LocaleHelper {
    ...
    /** Apply saved locale and return the new context. Call from Activity.attachBaseContext(). */
    ...
    fun getDarkMode(context: Context): Int =
        context.getSharedPreferences("app_prefs", Context.MODE_PRIVATE)
            .getInt(PREF_DARK_MODE, -1)   // -1 = MODE_NIGHT_FOLLOW_SYSTEM

    fun saveDarkMode(context: Context, mode: Int) {
        context.getSharedPreferences("app_prefs", Context.MODE_PRIVATE)
            .edit().putInt(PREF_DARK_MODE, mode).apply()
    }
}
